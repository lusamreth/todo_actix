...
use mongodb::{error::Error, results};
use serde::Serialize;
pub mod resperror;
use resperror::DBERROR;
#[async_trait(?Send)]
pub trait Itaskresp {
    async fn insert_task<T>(&self, entity: T) -> InsertRes where T:Serialize;
...
#[async_trait(?Send)]
pub trait ITodoresp {
    async fn insert_todo<T>(&self, entity: T) -> InsertRes where T:Serialize;
    async fn update_todo<T>(&self, new_entity: T, id: &str) -> UpdateRes where T:Serialize;
