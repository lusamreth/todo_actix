...
use resporitory_interface::{DeleteRes, DocRes, InsertRes, Todoid, UpdateRes,resperror};
...
impl ITodoresp for Db{
    ...
    async fn delete(&self,id:Todoid<'_>) -> DeleteRes {
        let query = doc! {"id":id};
        let common_write_con = options::WriteConcern::builder().w_timeout(Duration::from_millis(200)).build();
        let del_opt = options::DeleteOptions::builder().write_concern(common_write_con).build();
    ...
    async fn clear_all(&self) -> DeleteRes {
        ...
        let clear_opt = options::DeleteOptions::builder().write_concern(common_write_con).build();
    ...
    async fn insert_todo<T: serde::Serialize>(&self,entity:T) -> InsertRes {
        ...
        if let Bson::Document(new_doc) = new_doc{
            ...
            Err(DBERROR::Bson(Some(String::from("Invalid bson conversion type in creation of todo!"))))
        }
    }
    async fn update_todo<T: serde::Serialize>(&self,new_entity:T,id:Todoid<'_>) -> UpdateRes {
        ...
        if let Bson::Document(update_doc) = new_doc{
            ...
            Err(DBERROR::Bson(Some(String::from("Invalid bson conversion during update!"))))
        }
    }
}
