...
fn parse_time(time:&str) -> Vec<Result<i64,Fielderror>> {
    let mmhhss_format = regex::Regex::new(r"^(?:(?:([01]?\d|2[0-3]):)?([0-5]?\d):)?([0-5]?\d)$").unwrap();
    let captured = mmhhss_format.captures(time).unwrap().iter().skip(1).enumerate().map(|(idx, matched)|{
        match matched {
            Some(matched) => {
...
pub fn to_usecase_time(time:String) -> Result<UsecaseTime,Vec<Fielderror>>{
    ...
    let int_vec = vec_time.into_iter().enumerate().map(|(_idx,t_res)| {
...
fn test_time_parser(){
    ...
    let captured = mmhhss_format.captures(time).unwrap().iter().skip(1).map(|e|{
         match e {
             Some(matched) => {
                let str = matched.as_str();
                let parse_index = |s:&str|{
...
pub trait DefaultOutput<R:Serialize>{
    fn json_output(self,id:Option<String>) ->  OutputJson<Self,R> where Self: std::marker::Sized, Self:Serialize{
        OutputJson{
