...
use crate::usecase::{task_usecase::{task_finder,make_task,edit_task, Output,delete_task}};
...
pub async fn edit_task_api(body:web::Json<UpdateTaskJson>) -> impl Responder{
    ...
    let mut exec_inp = None;    

    let target = body.target_id.clone();
    let exec = ||async {
        let execute = edit_task::build_executor(exec_inp).await;
        let edition_res:Result<Output<()>, PortException> = execute(Box::new(gtw),body.fields.clone(),target).await;
        ...
        match time {
            Some(time) => {
               ...
               match time_res {
                   Ok(converted_time) => {
                    ...
                    exec_inp = Some(task_d);
                    let res:actix_web::HttpResponse = exec().await;
                    return res;
                   }
                   Err(fields) => {
            ...
            None => {
                let append_field = Fielderror{
                    ...
                    detials:"Failed Semantic in payload! Cannot complete without complete_time".to_string()
                };
                let input_err = PortError::External(String::new()).extend_input().invalid_input(vec![append_field]);
                input_err
            }
        }
    }else{
...
pub async fn list_tasks_api() -> impl Responder{
    ...
    match listing {
        ...
        Err(op_err) => {
            let res_err = op_err.into_iter().map(|e|{
                let hmm = e.into();
                return hmm;
